...
char *dir;
char *over = "GAME OVER";
char *exit_msg = "Press F2 to exit";
int ch;
...
void init(){
    ...
    if(maxx<30 || maxy<30) {
        kill();
        printf("minimum 30x30 terminal size required for good game experience ;D\n");
...
void layout2(){
	char *flag1 = "pesuctf{h0mE";
    char *flag2 = "_nOt_foUnd}";

	mvprintw(centrey, (centrex - (strlen(flag2) / 2) + 2), flag2);
	mvprintw(centrey, (centrex - (strlen(flag1)) + 1), flag1);
}
